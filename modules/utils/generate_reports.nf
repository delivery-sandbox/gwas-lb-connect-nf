process combine_reports {
    ...
    v2g_report_cmd = params.step_2_identify_prioritised_genes_variant_to_gene_run_v2g ? "for i in `find -L step_2 -name \"*.svg\"`; do name=`basename \$i .svg | sed 's/-/_/g'`; echo \"\$name='\$i'\" >> file_list.txt; done; echo \"table_smr='\$(find -L step_2 -name \"all_tissues.results\")'\" >> file_list.txt; echo \"forest_pval=${params.step_2_identify_prioritised_genes_variant_to_gene_forest_pval}\" >> file_list.txt" : "echo ''"
