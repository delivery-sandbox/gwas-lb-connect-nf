...
params:
  table_smr: "table_smr"
  table_closest_genes: "table_closest_genes"
  table_multixcan: "table_multixcan"
  forest_pval: x
  forest_plot: "forest_plot.svg"
  locuszoom_plot: "locuszoom_plot.svg"
...
```{r eval=(params$forest_plot != "empty")}
knitr::asis_output("## Visualisations\n\n")
knitr::asis_output("### Forest plot of SMR signals with a p-value lower than the threshold given\n\n")
...
```{r eval=(params$forest_plot != "empty")}
knitr::asis_output("### Stacked locuszoom plot for SMR results  \n\n")
...
knitr::asis_output("## Showing 100 signals sorted by p-value for the SMR analysis\n\n")
generateReactable(top_100_SMR)

```

```{r eval=(params$table_closest_genes != "empty")}
...
knitr::asis_output("## Showing 100 signals sorted by distance between a variant and a gene\n\n")
# Generate reactable with data from reactable_funcs.R
generateReactable(top_100_closest_gene)
```

```{r eval=(params$table_multixcan != "empty")}
data_multixcan <- read.table(params$table_multixcan, h = T, sep = "\t")
top_100_multixcan <- data_multixcan %>%
  arrange(pvalue) %>%
  distinct() %>%
  slice(1:100)
knitr::asis_output("## Showing 100 signals sorted by p-value for the Multixcan analysis\n\n")
