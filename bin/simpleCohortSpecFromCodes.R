...
input_file <- tibble(Phentype_Short = phenotype_name, Criteria_Ontology = vocabulary, Criteria = c(inclusion, exclusion), Is_Inclusion_Criteria = rep(T, length(inclusion), rep(F, length(exclusion))), Is_Exclusion_Criteria = rep(F, length(inclusion), rep(T, length(exclusion))))
